...
const validSources = [
    {
        name: "YouTube",
        domains: "youtube.com",
        regex: /^((?:https?:)?\/\/)?((?:www|m|music)\.)?((?:youtube(-nocookie)?\.com|youtu.be))(\/(?:[\w\-]+\?v=|embed\/|v\/)?)([\w\-]+)(\S+)?$/i
    },
    {
        name: "SoundCloud",
        domain: "soundcloud.com",
        regex: /^(?:(https?):\/\/)?(?:(?:www|m)\.)?(soundcloud\.com|snd\.sc)\/(.*)$/
    }
];
            ...
            for (let source of validSources) {
                ...
                if (i === validSources.length - 1) {
                    return new MyroError({
                        message: `${url} is not a valid link.\nCurrently supported platforms are ${validSources
