...
declare module "telegraf" {
  class Context {
    ...
    replyToWithAudio(this: Context, file: ReadStream, filename: string): Promise<Message.AudioMessage>;
  }
}
...
Context.prototype.replyTo = function (
  ...
  if (!confirmContext(this)) return Promise.reject(new Error("No context"));
  return this.telegram.sendMessage(this!.chat!.id, StringUtils.escapeMarkdownV2(content), {
...
Context.prototype.replyToWithAudio = function (
  ...
  if (!confirmContext(this)) return Promise.reject(new Error("No context"));
  this.telegram.sendChatAction(this.chat!.id, "upload_document").catch(() => { }); // ignore errors
  return this.telegram.sendAudio(this.chat!.id, {
    source: file,
    filename
  }, {
    reply_to_message_id: this.message?.message_id,
